...
export default function MarketplacePage() {
  ...
  const handleInvest = async (harvestId: string) => {
    ...
    if (amount < harvest.minimumInvestment) {
      alert(`Minimum investment is ${formatCurrency(harvest.minimumInvestment)}`);
      return;
    }

    if (amount > (harvest.fundingGoal - harvest.fundedAmount)) {
      alert(`Amount exceeds remaining funding need of ${formatCurrency(harvest.fundingGoal - harvest.fundedAmount)}`);
      return;
    }

    try {
      // Simple local update - simplified for now
      const updatedCampaigns = harvestCampaigns.map((item): Harvest => {
        if (item.id === harvestId) {
          const newFundedAmount = item.fundedAmount + amount;
          // Explicitly type the status as HarvestStatus
          const newStatus: HarvestStatus = newFundedAmount >= item.fundingGoal ? "growing" : "funding";
          ...
          return {
            ...
            investors: [
              ...item.investors,
              {
                ...
                userName: currentUser?.name || "Anonymous Investor",
                amount: amount,
              }
            ]
          };
      ...
      alert(`Successfully invested ${formatCurrency(amount)} in ${harvest.title}!`);
    } catch (error) {
      console.error(error);
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
              ...
              const riskLevel = getRiskLevel(harvest);
                ...
                return (
                  ...
                  >
                    <div className="relative h-52 overflow-hidden bg-[#EAD9B8]">
                      <Image
